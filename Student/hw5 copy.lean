...
def not_either_not_both { jam cheese } :
  ((no jam) ⊕ (no cheese)) → 
  (no (jam × cheese)) 
| Sum.inl nojam => (fun jch => nojam jch.1)
| Sum.inr nocheese => (fun jch => nocheese jch.2)
...
| (Sum.inl noa) => ab : α × β, noa ab.1
| (Sum.inr nob) => ab : α × β, nob ab.2
...
| noaorb => (a : α, noaorb (Sum.inl a), b : β, noaorb (Sum.inr b))
...
| (noα, noβ) (Sum.inl a) => noα a
| (noα, noβ) (Sum.inr b) => noβ b
...
def d3 := Doll.shell (Doll.shell (Doll.shell Doll.empty))
