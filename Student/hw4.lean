...
| (Sum.inr (Sum.inl b)) => (Sum.inl (Sum.inl a))
| (Sum.inr (Sum.inl c))=> (Sum.inr c)
...
-- Here:
 def prod_dist_sum_reverse {α β γ : Type} : (α × β) ⊕ (α × γ) → α x (β ⊕ y)
 | (Sum.inl (a,b)) => (a, Sum.inl b)
 | (Sum.inr (b,y)) => (a, Sum.inr c)
...
|(Sum.inl rain), rain2wet, _ => ran2wet rain
|(Sum.inr sprinkler), _, sprinkler2wet => sprinkler2wet sprinkler
